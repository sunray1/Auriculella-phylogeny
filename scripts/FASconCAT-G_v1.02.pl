...
sub start{
	...
	sub input_check{
		...
		for my $file ( sort @$aref_inputfile_list ){
			...
			if ( $file =~ /^.*\.aln$/             ){ &aln2fas ( \$file, \%sequence_of_taxon ) }	# READ IN CLUSTAL files (.aln)
			if ( $file =~ /^.*\.phy$/             ){ &phy2fas ( \$file, \%sequence_of_taxon ) }	# READ IN PHYLIP  files (.phy)
			if ( $file =~ /^.*\.fas$|^.*\.FASTA$/ ){ &fasEDIT ( \$file, \%sequence_of_taxon ) }	# READ IN FASTA   files (.fas || .FASTA)
			##############################
